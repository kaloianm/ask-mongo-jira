...
async def main():
    ...
    try:
        ...
        with open('epic_duration_vs_catalog_data.csv', 'w', newline='') as csvfile:
            fieldnames = [
                'epic_key', 'name', 'duration_weeks', 'catalog_percentage', 'catalog_crud',
                'catalog_ddl', 'catalog_implementation_change', 'catalog_total', 'data_movement',
                'something_else', 'total_classifications', 'start_date', 'end_date'
            ]
            ...
            for row in graph1_data:
                writer.writerow({
                    ...
                    row['epic_key'],
                    'name':
                    row['name'][:100],  # Truncate long names
                    'duration_weeks':
                    round(row['duration_weeks'], 2),
                    'catalog_percentage':
                    round(row['catalog_percentage'], 2),
                    'catalog_crud':
                    row['catalog_crud'],
                    'catalog_ddl':
                    row['catalog_ddl'],
                    'catalog_implementation_change':
                    row['catalog_implementation_change'],
                    'catalog_total':
                    row['catalog_total'],
                    'data_movement':
                    row['data_movement'],
        ...
        if graph1_data:
            ...
            print(
                f"- Average catalog percentage: {sum(catalog_percentages)/len(catalog_percentages):.1f}%"
            )

            # Show top 5 longest and shortest epics
            sorted_by_duration = sorted(graph1_data,
                                        key=lambda x: x['duration_weeks'],
                                        reverse=True)
            print(f"\nTop 5 longest epics:")
            for epic in sorted_by_duration[:5]:
                print(
                    f"  {epic['epic_key']}: {epic['duration_weeks']:.1f} weeks, {epic['catalog_percentage']:.1f}% Catalog"
                )
