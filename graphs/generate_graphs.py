...
class GraphDataGenerator:
    ...
    async def get_epic_duration_vs_crud_percentage(self) -> List[Dict[str, Any]]:
                ...
                '$group': {
                    ...
                    'catalog_implementation_change': {
                        '$sum': {
                            '$cond': [{
                                '$eq':
                                ['$analyses.classification', 'Catalog Implementation Change']
                            }, 1, 0]
                        }
                    },
                    'data_movement': {
        ...
        async for doc in cursor:
            ...
            try:
                ...
                logger.warning(
                    f"Skipping epic {doc.get('epic_key')} due to date parsing error: {e}")
                continue
                ...
                '$project': {
                    ...
                    'bucket': {
                        '$switch': {
                            'branches': [{
                                'case': {
                                    '$lte': ['$catalog_percentage', 10]
                                },
                                'then': '0-10%'
                            }, {
                                'case': {
                                    '$lte': ['$catalog_percentage', 30]
                                },
                                'then': '10-30%'
                            }, {
                                'case': {
                                    '$lte': ['$catalog_percentage', 50]
                                },
                                'then': '30-50%'
                            }],
                            'default':
    ...
    def generate_graph_1(self, data: List[Dict[str, Any]]):
        ...
        epic_names = [
            f"{item['epic_key']}: {item['name'][:30]}..."
            if len(item['name']) > 30 else f"{item['epic_key']}: {item['name']}" for item in data
        ]
    ...
    def generate_graph_2(self, data: List[Dict[str, Any]]):
        ...
        plt.title('Distribution of Epics by Catalog Change Percentage',
                  fontsize=14,
                  fontweight='bold')
        ...
        for item in data:
            ...
            if len(bucket_examples[bucket]) < 3:  # Show up to 3 examples per bucket
                bucket_examples[bucket].append(
                    f"{item['epic_key']} ({item['catalog_percentage']:.1f}%)")
